...
public class LockScreenActivity extends Activity implements LockScreenCallbacks, OnFocusChangeListener, ICacheWordSubscriber
{
	...
	{
		super.onStart();
		info.guardianproject.zt.App.getInstance().onLockScreenResumed(this);
	...
	{
		super.onStop();
		info.guardianproject.zt.App.getInstance().onLockScreenPaused(this);
	...
	public boolean onKeyDown(int keyCode, KeyEvent event)
	{
		if ((keyCode == KeyEvent.KEYCODE_BACK))
		{
			...
			LocalBroadcastManager.getInstance(this).sendBroadcastSync(new Intent(info.guardianproject.zt.App.EXIT_BROADCAST_ACTION));
			finish();
			return true;
		}
	...
	private void createFirstTimeView()
	{
		...
		if (info.guardianproject.zt.App.UI_ENABLE_LANGUAGE_CHOICE)
		{
			...
			selectLanguageByUiLanguage(info.guardianproject.zt.App.getSettings().uiLanguage(), false);
			mDropdownLanguage.setOnSelectionChangedListener(new DropdownSpinner.OnSelectionChangedListener() {

				@Override
				public void onSelectionChanged(int position) {
					info.guardianproject.zt.App.getSettings().setUiLanguage(mLanguageCodes[position]);
				}
			});
		}
		else
	...
	private void createLockView()
	{
		...
		mBtnOpen.setOnClickListener(new OnClickListener()
		{
			...
			public void onClick(View v)
			{
				...
				if (info.guardianproject.zt.App.getSettings().useKillPassphrase() && mEnterPassphrase.getText().toString().equals(info.guardianproject.zt.App.getSettings().killPassphrase()))
				{
					// Kill password entered, wipe!
					info.guardianproject.zt.App.getInstance().wipe(SocialReader.DATA_WIPE);
				...
				try {
					...
					int failedAttempts = info.guardianproject.zt.App.getSettings().currentNumberOfPasswordAttempts();
					failedAttempts++;
					info.guardianproject.zt.App.getSettings().setCurrentNumberOfPasswordAttempts(failedAttempts);
					if (failedAttempts == info.guardianproject.zt.App.getSettings().numberOfPasswordAttempts())
					{
