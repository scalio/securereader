...
public class LockableActivity extends ActionBarActivity implements OnSharedPreferenceChangeListener
{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		LocalBroadcastManager.getInstance(this).registerReceiver(mLockReceiver, new IntentFilter(info.guardianproject.zt.App.LOCKED_BROADCAST_ACTION));
		LocalBroadcastManager.getInstance(this).registerReceiver(mUnlockReceiver, new IntentFilter(info.guardianproject.zt.App.UNLOCKED_BROADCAST_ACTION));
	...
	{
		if (!mLockedInOnPause)
			info.guardianproject.zt.App.getInstance().onActivityResume(this);
		super.onStart();
	}
	...
	protected void onStop()
	{
		super.onStop();
		if (!mLockedInOnPause)
			info.guardianproject.zt.App.getInstance().onActivityPause(this);
	}
	...
	{
		info.guardianproject.zt.App.getSettings().registerChangeListener(this);
	}

	private void removeSettingsChangeListener()
	{
		info.guardianproject.zt.App.getSettings().unregisterChangeListener(this);
	}
	
	@Override
	public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key)
	{
		if (info.guardianproject.zt.SettingsUI.KEY_ENABLE_SCREENSHOTS.equals(key))
		{
			if (LOGGING) 
	...
	{
		if (info.guardianproject.zt.App.getSettings().enableScreenshots())
		{
