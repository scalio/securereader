package info.guardianproject.zt.publish.db;
...
public class StoryMakerDB extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        db.execSQL(Schema.Projects.CREATE_TABLE_PROJECTS);
        db.execSQL(Schema.Scenes.CREATE_TABLE_SCENES);
		db.execSQL(Schema.Lessons.CREATE_TABLE_LESSONS);
		db.execSQL(Schema.Media.CREATE_TABLE_MEDIA);
		db.execSQL(Schema.Auth.CREATE_TABLE_AUTH);
        db.execSQL(Schema.Tags.CREATE_TABLE_TAGS);
        db.execSQL(Schema.Jobs.CREATE_TABLE_JOBS);
        db.execSQL(Schema.PublishJobs.CREATE_TABLE_PUBLISH_JOBS);
    }
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        if ((oldVersion < 2) && (newVersion == 2)) {
            db.execSQL(Schema.Projects.UPDATE_TABLE_PROJECTS);
        } 
        if ((oldVersion < 3) && (newVersion == 3)) {
            db.execSQL(Schema.Media.UPDATE_TABLE_MEDIA_ADD_TRIM_START);
            db.execSQL(Schema.Media.UPDATE_TABLE_MEDIA_ADD_TRIM_END);
            db.execSQL(Schema.Media.UPDATE_TABLE_MEDIA_ADD_DURATION);
        } 
        if ((oldVersion < 4) && (newVersion >= 4)) {
            db.execSQL(Schema.Auth.UPDATE_TABLE_AUTH);
        }
        if ((oldVersion < 5) && (newVersion >= 5)) {
            db.execSQL(Schema.Projects.UPDATE_TABLE_PROJECTS_ADD_CREATED_AT);
            db.execSQL(Schema.Projects.UPDATE_TABLE_PROJECTS_ADD_UPDATED_AT);
            db.execSQL(Schema.Scenes.UPDATE_TABLE_SCENES_ADD_CREATED_AT);
            db.execSQL(Schema.Scenes.UPDATE_TABLE_SCENES_ADD_UPDATED_AT);
            db.execSQL(Schema.Media.UPDATE_TABLE_MEDIA_ADD_CREATED_AT);
            db.execSQL(Schema.Media.UPDATE_TABLE_MEDIA_ADD_UPDATED_AT);
        }
        if ((oldVersion < 6) && (newVersion >= 6)) {
            db.execSQL(Schema.Projects.UPDATE_TABLE_PROJECTS_ADD_SECTION);
            db.execSQL(Schema.Projects.UPDATE_TABLE_PROJECTS_ADD_LOCATION);
            db.execSQL(Schema.Tags.UPDATE_TABLE_TAGS);
        }
        if ((oldVersion < 7) && (newVersion >= 7)) {
        }
        if ((oldVersion < 8) && (newVersion >= 8)) {
            db.execSQL(Schema.Projects.UPDATE_TABLE_PROJECTS_ADD_DESCRIPTION);
        }
