...
public final class Font {
	...
	private final static String szLamAndAlef = Character.toString((char) 0xfedf) + Character.toString((char) 0xfe8e); // Lam + Alef
	
	private final static String szLamStickAndAlef = Character.toString((char) 0xfee0) + Character.toString((char) 0xfe8e); // Lam (Sticky !!!)+
																															// Alef
	...
	private final static String szLamAndAlefWoosim = Character.toString((char) 0xe1) + Character.toString((char) 0xbb); // Lam + Alef
	
	private final static String szLamStickAndAlefWoosim = Character.toString((char) 0x90) + Character.toString((char) 0xbb); // Lam (Sticky !!!)+
																																// Alef
	
	private final static String szLaWoosim = Character.toString((char) 0xd9); // La
	private final static String szLaStickWoosim = Character.toString((char) 0xd9); // La
	...
	// (char) 0xfbff, (char) 0xfbfc), // Font yeh
	...
	private static final String ArabicReverse(String s) {
		try {
			...
			while (i < s.length()) {
				...
				{
					rev = "";
					while (i < s.length() && ((chs[i] >= '0' && chs[i] <= '9') || chs[i] == '/')) // isDigit(s[i])
																									// ?
					{
						rev = rev + chs[i];
						++i;
	...
	public static final String Convert(String In) {
		...
		try {
			...
			for (int i = 0; i < In.length(); i++) {
				/* WCHAR */
				char ch = chIn[i];
				if ((ch >= (char) 0x0621 && ch <= (char) 0x064a) || (ch == (char) 0x067e) || (ch == (char) 0x0686) || (ch == (char) 0x0698) || (ch == (char) 0x06a9) || (ch == (char) 0x06af) || (ch == (char) 0x06cc) || (ch == (char) 0x06c0)) // is an Arabic character?
				{
					int idx = 0;
    ...
    private static Typeface typefaceBd;

    public static final Typeface getMitraFont(Context act) {
        if (typeface == null) {
    ...
    public static final Typeface getMitraBdFont(Context act) {
        if (typefaceBd == null) {
            typefaceBd = Typeface.createFromAsset(act.getAssets(),
                    "mitra_bd.ttf");
        }
        return typefaceBd;
    }
